  ...
  if (dataSend.language === "vi") {
    ...
    result = `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e5e5;">
        ...
        <div style="padding: 32px 24px;">
          <p style="margin: 0 0 24px 0; font-size: 16px;">Xin chào <strong>${
            dataSend.userName
          }</strong>,</p>
          <p style="margin: 0 0 32px 0; color: #666; line-height: 1.5;">
            Cảm ơn bạn đã đặt phòng tại ${dataSend.typeData.valueVi} <strong>${
      dataSend.propertyName
    }</strong> 
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 32px;">
            <tr>
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0; font-weight: 500; width: 30%;">${
                dataSend.typeData.valueVi
              }:</td>
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;"><strong>${
                dataSend.propertyName
              }</strong></td>
            </tr>
            <tr>
              ...
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;">${
                dataSend.propertyAddress
              }</td>
            </tr>
            <tr>
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0; font-weight: 500;">Nhận phòng:</td>
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;">
                <strong>${checkInFormatted}</strong> (từ ${
      dataSend.checkInTimeData?.valueVi
    } giờ)
              </td>
            </tr>
            <tr>
              ...
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;">
                <strong>${checkOutFormatted}</strong> (trước ${
      dataSend.checkOutTimeData?.valueVi
    } giờ)
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td style="padding: 12px 0;">
                <strong style="color: #2563eb; font-size: 18px;">${Number(
                  dataSend.totalPrice
                ).toLocaleString("vi-VN")} VND</strong>
              </td>
            </tr>
          </table>
          ...
          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
            ...
            <div style="text-align: center;">
              ...
              }" target="_blank" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 16px;">
                Xác nhận đặt phòng
              </a>
            </div>
          </div>
    ...
    result = `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e5e5;">
        ...
        <!-- Content -->
        <div style="padding: 32px 24px;">
          <p style="margin: 0 0 24px 0; font-size: 16px;">Hello <strong>${
            dataSend.userName
          }</strong>,</p>
          <p style="margin: 0 0 32px 0; color: #666; line-height: 1.5;">
            Thank you for booking at ${
              dataSend.typeData.valueEn || dataSend.typeData.valueVi
            } <strong>${dataSend.propertyName}</strong> 
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 32px;">
            ...
            <tr>
              ...
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;">
                <strong>${checkInFormatted}</strong> (from ${
      dataSend.checkInTimeData?.valueEn || dataSend.checkInTimeData?.valueVi
    })
              </td>
            </tr>
            <tr>
              ...
              <td style="padding: 12px 0; border-bottom: 1px solid #f0f0f0;">
                <strong>${checkOutFormatted}</strong> (before ${
      dataSend.checkOutTimeData?.valueEn || dataSend.checkOutTimeData?.valueVi
    })
              </td>
            </tr>
            <tr>
...
const sendCheckoutEmail = async (dataSend) => {
  // Create a test account or replace with real credentials.
  const transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 587,
  ...
  const info = await transporter.sendMail({
    from: "ROOM.BOOKING",
    to: dataSend.reciverEmail,
    subject: "Mời bạn đánh giá trải nghiệm lưu trú",
    html: getBodyHTMLCheckOutEmail(dataSend),
  });
};

const getBodyHTMLCheckOutEmail = (dataSend) => {
  let result = "";
  ...
  <div style="font-family: 'Segoe UI', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e5e5;">
    ...
    <div style="background: #2563eb; color: white; padding: 24px; text-align: center;">
      <h2 style="margin: 0; font-size: 24px; font-weight: 600;">Cảm ơn bạn đã lưu trú</h2>
      <p style="margin: 8px 0 0 0; font-size: 14px; opacity: 0.9;">ROOM.BOOKING</p>
    </div>
    
    <div style="padding: 32px 24px;">
      <p style="margin: 0 0 24px 0; font-size: 16px;">Chào bạn <strong>${dataSend.userName}</strong>,</p>
      <p style="margin: 0 0 32px 0; color: #666; line-height: 1.5;">
        ROOM.BOOKING xin chân thành cảm ơn bạn đã lựa chọn lưu trú tại <strong>${dataSend.propertyName}</strong>.
        Chúng tôi hy vọng bạn đã có một trải nghiệm thật tuyệt vời!
      </p>
      ...
      <p style="margin: 0 0 24px 0; color: #666; line-height: 1.5;">
        Ý kiến của bạn rất quan trọng để chúng tôi có thể cải thiện chất lượng dịch vụ. Vui lòng dành chút thời gian để lại đánh giá về kỳ nghỉ của bạn.
      </p>
      
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
        <p style="margin: 0 0 16px 0; color: #334155; text-align: center;">
